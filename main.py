...
def process_fund_data(original_name: str, url: str) -> Tuple[str, str, str, str]:
    """ファンドデータを取得し、デバッグ情報を出力した後、ファンドデータを返す"""
    # 各URLに対して、ファンドデータを取得
    fund_name, performance, method, csv_path = scrape_fund_data(url)
    # デバッグ情報の出力
    log_debug(f"Processed: {original_name}")
    log_debug(f"  Fund Name: {fund_name} (Method: {method})")
